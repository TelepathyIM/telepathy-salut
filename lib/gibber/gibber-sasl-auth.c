...
gibber_sasl_auth_class_init (GibberSaslAuthClass *gibber_sasl_auth_class)
{
  ...
  g_type_class_add_private (gibber_sasl_auth_class,
      sizeof (GibberSaslAuthPrivate));
...
gibber_sasl_auth_mechanisms_to_list (GibberXmppNode *mechanisms)
{
  ...
  gibber_xmpp_node_each_child (mechanisms, each_mechanism, &result);
  return result;
}
    ...
    GibberSaslAuth *sasl)
{
  ...
  if (strcmp (
      gibber_xmpp_node_get_ns (stanza->node), GIBBER_XMPP_NS_SASL_AUTH))
    {
      auth_failed (sasl, GIBBER_SASL_AUTH_ERROR_INVALID_REPLY,
...
gibber_sasl_auth_start_mechanism (GibberSaslAuth *sasl,
    GibberSaslAuthMechanism mech, GError **error)
{
  GibberXmppStanza *stanza;
  GibberSaslAuthPrivate *priv = GIBBER_SASL_AUTH_GET_PRIVATE(sasl);
  GError *send_error = NULL;
...
gibber_sasl_auth_authenticate (GibberSaslAuth *sasl,
    const gchar *server, GibberXmppConnection *connection,
    GibberXmppStanza *features, gboolean allow_plain, GError **error)
{
  GibberSaslAuthPrivate *priv = GIBBER_SASL_AUTH_GET_PRIVATE (sasl);
  ...
  mech_node = gibber_xmpp_node_get_child_ns (features->node, "mechanisms",
      GIBBER_XMPP_NS_SASL_AUTH);
