...
#define GIBBER_TCP_TRANSPORT_GET_PRIVATE(o)     (G_TYPE_INSTANCE_GET_PRIVATE ((o), GIBBER_TYPE_TCP_TRANSPORT, GibberTCPTransportPrivate))
...
static void
gibber_tcp_transport_class_init (GibberTCPTransportClass *gibber_tcp_transport_class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (gibber_tcp_transport_class);

  g_type_class_add_private (gibber_tcp_transport_class, sizeof (GibberTCPTransportPrivate));
    ...
    const gchar *host, const gchar *port) 
{
  ...
  while (tmpaddr != NULL) {
    ...
    } else if ((ret = connect(fd, tmpaddr->ai_addr, tmpaddr->ai_addrlen))
               < 0) {
