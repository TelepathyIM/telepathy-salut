...
#define GIBBER_XMPP_WRITER_GET_PRIVATE(o) \
  (G_TYPE_INSTANCE_GET_PRIVATE ((o), GIBBER_TYPE_XMPP_WRITER, \
...
gibber_xmpp_writer_class_init (GibberXmppWriterClass *gibber_xmpp_writer_class)
{
  ...
  g_type_class_add_private (gibber_xmpp_writer_class,
      sizeof (GibberXmppWriterPrivate));
...
void
gibber_xmpp_writer_stream_close (GibberXmppWriter *writer,
    const guint8 **data, gsize *length)
{
