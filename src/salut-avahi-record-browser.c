/*
 * salut-avahi-record-browser.c - Source for SalutAvahiRecordBrowser
...
G_DEFINE_TYPE(SalutAvahiRecordBrowser, salut_avahi_record_browser, G_TYPE_OBJECT)
...
/* private structure */
typedef struct _SalutAvahiRecordBrowserPrivate SalutAvahiRecordBrowserPrivate;

struct _SalutAvahiRecordBrowserPrivate
{
...
#define SALUT_AVAHI_RECORD_BROWSER_GET_PRIVATE(o)     (G_TYPE_INSTANCE_GET_PRIVATE ((o), SALUT_TYPE_AVAHI_RECORD_BROWSER, SalutAvahiRecordBrowserPrivate))

static void
salut_avahi_record_browser_init (SalutAvahiRecordBrowser *obj)
{
  /* allocate any data required by the object here */
}

static void salut_avahi_record_browser_dispose (GObject *object);
static void salut_avahi_record_browser_finalize (GObject *object);

static void
salut_avahi_record_browser_set_property (GObject *object,
                                 guint property_id,
                                 const GValue *value,
                                 GParamSpec *pspec) {
...
static void
salut_avahi_record_browser_get_property (GObject *object,
                                 guint property_id,
                                 GValue *value,
                                 GParamSpec *pspec) {
  SalutAvahiRecordBrowser *browser = SALUT_AVAHI_RECORD_BROWSER(object);
...
static void
salut_avahi_record_browser_class_init (SalutAvahiRecordBrowserClass *salut_avahi_record_browser_class)
{
  GObjectClass *object_class = G_OBJECT_CLASS (salut_avahi_record_browser_class);
  GParamSpec *param_spec;

  g_type_class_add_private (salut_avahi_record_browser_class, sizeof (SalutAvahiRecordBrowserPrivate));

  object_class->dispose = salut_avahi_record_browser_dispose;
  object_class->finalize = salut_avahi_record_browser_finalize;
  ...
  signals[NEW] = 
    g_signal_new("new-record",
                 ...
                 0,
                 NULL, NULL,
                 salut_avahi_record_browser_marshal_VOID__INT_INT_STRING_UINT_UINT_POINTER_INT_INT,
                 G_TYPE_NONE, 8,
                 G_TYPE_INT,
...
void
salut_avahi_record_browser_dispose (GObject *object)
{
  SalutAvahiRecordBrowser *self = SALUT_AVAHI_RECORD_BROWSER (object);
  SalutAvahiRecordBrowserPrivate *priv = SALUT_AVAHI_RECORD_BROWSER_GET_PRIVATE (self);
  ...
  if (G_OBJECT_CLASS (salut_avahi_record_browser_parent_class)->dispose)
    G_OBJECT_CLASS (salut_avahi_record_browser_parent_class)->dispose (object);
}

void
salut_avahi_record_browser_finalize (GObject *object)
{
  SalutAvahiRecordBrowser *self = SALUT_AVAHI_RECORD_BROWSER (object);
  SalutAvahiRecordBrowserPrivate *priv = SALUT_AVAHI_RECORD_BROWSER_GET_PRIVATE (self);
  ...
  G_OBJECT_CLASS (salut_avahi_record_browser_parent_class)->finalize (object);
}


SalutAvahiRecordBrowser *
salut_avahi_record_browser_new(const gchar *name, guint16 type) {
  return salut_avahi_record_browser_new_full(AVAHI_IF_UNSPEC,
...
salut_avahi_record_browser_new_full(AvahiIfIndex interface, 
  ...
  return g_object_new(SALUT_TYPE_AVAHI_RECORD_BROWSER,
                      "interface", interface,
                      "protocol", protocol,
                      "name",     name,
                      "class",    clazz,
...
static void
_avahi_record_browser_cb(AvahiRecordBrowser *r, AvahiIfIndex interface,
  ...
  switch (event) {
    ...
    case AVAHI_BROWSER_FAILURE: {
      ...
      error = g_error_new(SALUT_AVAHI_ERRORS,  aerrno,
                          "Browsing failed: %s", avahi_strerror(aerrno));
